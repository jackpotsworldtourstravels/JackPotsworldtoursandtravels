...
class Settings(BaseSettings):
    ...
    #: coturn's `static-auth-secret`. WHEN SET, THIS WINS over the static
    #: username/password below and every browser gets its own credential that
    #: expires. See `call_signaling.ice_config`. Generated by
    #: deploy/setup-coturn.sh, which writes the same value into turnserver.conf.
    turn_static_auth_secret: str = ""
    #: How long a minted TURN credential lasts. An hour is far longer than any
    #: call and short enough that a scraped credential is worthless by the time
    ...
    def turn_configured(self) -> bool:
        """TURN is usable if there are URLs AND some way to authenticate."""
        if not self.turn_url_list:
            return False
        return bool(
            self.turn_static_auth_secret
            or (self.turn_username and self.turn_password)
        )
