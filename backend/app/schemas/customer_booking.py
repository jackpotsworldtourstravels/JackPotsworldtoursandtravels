...
class FlightCheckoutRequest(BaseModel):
    ...
    NOTE WHAT IS NOT HERE: no amount, no currency, no customer id, no fare, no
...
class FlightCheckoutResponse(BaseModel):
...
class FlightReconcileRequest(BaseModel):
...
class FlightReconcileResponse(BaseModel):
